...
**Status:** Live — updated to reflect production state (v5.6.0)
...
Ten chatbot IDs stored as WP options — one per distinct AI job. All ten are registered as integration slots via `stevegpt_plugin_integration_slots` (Super Strengths tab in **SteveGPT → Chatbots**) and are also editable on the plugin's own Configuration screen. All follow the `mfsd_stevegpt_map_ss_*` naming convention matching the rest of the platform.
...
| `mfsd_stevegpt_map_ss_game_summary` | Game strengths summary *(new in 5.6.0)* | `GET /game/summary` + guessing-game results | Student-only reflection on the cards their family wrote. Tokens: `student_name`, `strengths_list` |

**Slot tokens for the two slots registered in 5.6.0:** Welcome intro — `student_name`, `student_age`, `end_condition`; Welcome chat — none (chat mode).
...
All AI calls use the SteveGPT v8 pattern (`stevegtp/techspecs/MFSD_CHATBOT_IMPLEMENTATION_PATTERN.md`). As of v5.6.0 there are **no** remaining `simpleTextQuery()`, `$GLOBALS['stevegtp']` or `$GLOBALS['mwai']` calls in the plugin.

| Call point | Code | Slot option |
|---|---|---|
| Student / parent summaries | `MFSD_SS_Summary::generate_ai_summary()` → prompt built in PHP (`build_student_prompt` / `build_parent_prompt`) → `SteveGPT_Chatbot::get()->query()` | `ss_student_summary`, `ss_parent_summary` |
| Demo picker / demo summary | `MFSD_SS_Demo` → prompt built in PHP → `SteveGPT_Chatbot::get()->query()` | `ss_demo_picker`, `ss_demo_summary` |
| Welcome intro | `MFSD_SS_API::memory_intro()` → `MFSD_SS_Game::steve_task()` | `ss_welcome_intro` |
| Game summary (`GET /game/summary`) | `MFSD_SS_API::game_summary()` → `MFSD_SS_Game::steve_task()` | `ss_game_summary` |
| Guessing-game results | `MFSD_SS_Game::generate_strengths_summary()` → `steve_task()` | `ss_game_summary` |
| Chat widgets | `[stevegpt_chatbot id="…" context="…"]` shortcode | `*_chat` slots |

### 13.0 `MFSD_SS_Game::steve_task()` helper (v5.6.0)

```php
MFSD_SS_Game::steve_task(string $option, string $fallback_prompt, array $tokens, int $user_id): string
```

1. Reads the chatbot ID from `get_option($option)`. Returns `''` if the slot is empty or `SteveGPT_Chatbot` is unavailable (caller shows its static fallback).
2. `SteveGPT_Chatbot::get($id)`; if the chatbot has a `prompt_template`, the prompt is `render_prompt($tokens)`, otherwise the built-in `$fallback_prompt` is sent unchanged — so assigning a chatbot without a template keeps today's wording.
3. `query($prompt, $user_id)` — handles skills, student context and cost logging. Exceptions are logged as `[SuperStrengths] SteveGPT task error (<option>): …` and return `''`.

> **History:** before 5.6.0 the welcome intro used `$GLOBALS['stevegtp']->simpleTextQuery()`. That global is never set by `stevegtp`, so the intro always returned the fallback. The game summary and guessing-game summary used `$GLOBALS['mwai']->simpleTextQuery()` (AI Engine) and only worked while AI Engine was active.
>
> **Deployment note:** the `ss_game_summary` slot is new and starts empty — assign a chatbot in SteveGPT → Chatbots before/at deploy, otherwise those two summaries return no AI text.
...
| 5.6.0 | SteveGPT migration of the last 3 legacy AI calls: welcome intro (`memory_intro`, previously `$GLOBALS['stevegtp']` — never worked), game summary (`game_summary`) and guessing-game summary (`generate_strengths_summary`) (previously `$GLOBALS['mwai']`). New shared helper `MFSD_SS_Game::steve_task()`. Registered missing slots: Welcome intro, Welcome chat, and new **Game strengths summary** (`mfsd_stevegpt_map_ss_game_summary`, also added to admin Configuration). No DB or JS changes. |

---

*End of specification — v5.6.0 — MFSD Super Strengths Cards*
